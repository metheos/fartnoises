...
class JudgeReconnectionTester {
  ...
  private createPlayer(name: string): Promise<TestPlayer> {
    return new Promise((resolve, reject) => {
      ...
      socket.on("gamePausedForDisconnection", (data) => {
        this.log(`${name} received gamePausedForDisconnection: ${data.disconnectedPlayerName}`);
      });

      socket.on("reconnectionVoteRequest", (data) => {
        this.log(`${name} received vote request for ${data.disconnectedPlayerName}`);
      ...
      socket.on("reconnectionVoteResult", (data) => {
        this.log(`${name} received vote result: ${data.continueWithoutPlayer ? "continue" : "wait"} for ${data.disconnectedPlayerName}`);
      });
  ...
  private async joinRoom(player: TestPlayer, roomCode: string): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      player.socket.emit("joinRoom", roomCode, playerData, (success: boolean) => {
        if (success) {
          player.roomCode = roomCode;
          this.log(`Player ${player.name} joined room ${roomCode}`);
          resolve(true);
        } else {
          this.log(`Player ${player.name} failed to join room ${roomCode}`, true);
          resolve(false);
        }
      });
    });
  ...
  private async testGameSetup(): Promise<void> {
    ...
    try {
      ...
      this.failTest("Game Setup and Judge Identification", error instanceof Error ? error.message : "Unknown error");
    }
  }
  ...
  private async testJudgeDisconnectionWithWaitVote(): Promise<void> {
    ...
    try {
      ...
      this.players.filter(p => !p.isJudge).forEach(player => {
        player.socket.once("reconnectionVoteRequest", () => {
          votingOccurred = true;
          this.log(`${player.name} received vote request - setting votingOccurred to true`);
        });
        ...
        player.socket.once("gameResumed", () => {
          gameResumed = true;
          this.log(`${player.name} received gameResumed - setting gameResumed to true`);
        });
      });
      ...
      if (votingOccurred && voteResult === "wait") {
        this.log(`Voting phase completed correctly - players voted to "${voteResult}"`);
        this.passTest("Judge Disconnection with Wait Vote");
      } else {
        this.failTest("Judge Disconnection with Wait Vote", `Voting: ${votingOccurred}, Vote result: ${voteResult}, Game resumed: ${gameResumed}`);
      }
    } catch (error) {
      this.failTest("Judge Disconnection with Wait Vote", error instanceof Error ? error.message : "Unknown error");
    }
  }
  ...
  private async testJudgeReconnectionAndRoleRetention(): Promise<void> {
    ...
    try {
      ...
      newSocket.on("connect", () => {
        ...
        newSocket.emit("reconnectToRoom", this.roomCode, disconnectedJudge.name, disconnectedJudge.originalId, (success: boolean) => {
          if (success) {
      ...
      newSocket.on("judgeSelected", (judgeId) => {
        ...
        if (judgeId === newSocket.id) {
          ...
          this.log("Judge role preserved successfully via judgeSelected event!");
        }
      });
      ...
      newSocket.on("roomJoined", ({ room, player }) => {
        roomState = room;
        this.log(`Judge reconnected to room. Current judge: ${room.currentJudge}, Reconnected player ID: ${player.id}`);
        
        if (room && room.currentJudge === player.id) {
          judgeRolePreserved = true;
          this.log("Judge role confirmed in room state!");
        } else if (room && room.currentJudge) {
          this.log(`Judge role NOT preserved - current judge is ${room.currentJudge}, but reconnected player is ${player.id}`, true);
        } else {
      ...
      newSocket.on("roomUpdated", (room) => {
        this.log(`Room updated. Current judge: ${room.currentJudge}, Reconnected player ID: ${newSocket.id}`);
      ...
      this.log(`Final status: Reconnection: ${reconnectionSuccess}, Role preserved: ${judgeRolePreserved}`);
      if (roomState) {
        this.log(`Final room state - Current judge: ${roomState.currentJudge}, Player count: ${roomState.players?.length}`);
      }

      if (reconnectionSuccess && judgeRolePreserved) {
        this.passTest("Judge Reconnection and Role Retention");
      } else {
        this.failTest("Judge Reconnection and Role Retention", `Reconnection: ${reconnectionSuccess}, Role preserved: ${judgeRolePreserved}`);
      }
    } catch (error) {
      this.failTest("Judge Reconnection and Role Retention", error instanceof Error ? error.message : "Unknown error");
    }
  }
  ...
  public async runAllTests(): Promise<void> {
    try {
      ...
      this.log("Phase 2: Judge Disconnection with Wait Vote");
      await this.testJudgeDisconnectionWithWaitVote();
      await this.sleep(2000);

      this.log("Phase 3: Judge Reconnection and Role Retention");
      await this.testJudgeReconnectionAndRoleRetention();

      this.log("=== ALL TESTS COMPLETED ===");
    } catch (error) {
      this.log(`Test suite error: ${error instanceof Error ? error.message : "Unknown error"}`, true);
    } finally {
      this.cleanup();
      this.printResults();
    }
